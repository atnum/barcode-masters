...
public interface AsnRepository extends JpaRepository<ASN, String> {
	...
	@Query(value = "select distinct * from asn_view a Where UPPER(a.asn_nbr) LIKE %?1% OR LOWER(a.asn_nbr) LIKE %?1%", nativeQuery = true)
	List<ASN> findAllAsn(String asn);
	
	@Query(value = "select distinct * from asn_view a", nativeQuery = true)
	List<ASN> allASN();
	
	@Query(value = "select distinct a.asn_nbr from asn_view a Where UPPER(a.asn_nbr) LIKE %?1% OR LOWER(a.asn_nbr) LIKE %?1% ORDER BY 1 DESC", nativeQuery = true)
